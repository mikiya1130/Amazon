...
class Net(nn.Module):
    ...
    def __init__(self, CatDict):
        ...
        self.SqueezeUpsample.append(
            nn.Sequential(
                nn.Conv2d(
                    256 + 512, 256, stride=1, kernel_size=1, padding=0, bias=False
                ),
                nn.BatchNorm2d(256),
                nn.ReLU(),
            )
        ...
        self.SqueezeUpsample.append(
            nn.Sequential(
                nn.Conv2d(
                    256 + 256, 256, stride=1, kernel_size=1, padding=0, bias=False
                ),
                nn.BatchNorm2d(256),
                nn.ReLU(),
            )
        ...
        for f, nm in enumerate(CatDict):
            ...
            self.OutLayersDict[nm] = nn.Conv2d(
    ...
    ##########################################Run inference################################################################################################################
    def forward(
        self, Images, UseGPU=True, TrainMode=True, FreezeBatchNormStatistics=False
    ):
        ...
        InpImages = (
            torch.autograd.Variable(
                torch.from_numpy(Images.astype(float)), requires_grad=False
            )
            .transpose(2, 3)
            .transpose(1, 2)
            .type(tp)
        )
        if FreezeBatchNormStatistics == True:
            self.eval()
        ...
        for i, PSPLayer in enumerate(
            self.PSPLayers
        ):  # run PSP layers scale features map to various of sizes apply convolution and concat the results
        ...
        for nm in self.OutLayersDict:
            ...
            l = nn.functional.interpolate(
                l, size=InpImages.shape[2:4], mode="bilinear"
